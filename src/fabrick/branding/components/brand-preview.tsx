    ...
    >
      <div className="p-6">
        <div className="flex items-center gap-4">
          <div className="flex h-14 w-14 items-center justify-center rounded-2xl border bg-white/80 shadow-sm">
            {brand.identity.logoUrl ? (
              ...
              <span className="text-xl font-bold" style={{ color: brand.identity.secondaryColor }}>
                {brand.identity.name.slice(0, 1).toUpperCase()}
              </span>
            )}
          </div>
          <div>
            <h3 className="text-xl font-semibold tracking-tight">{brand.identity.name}</h3>
        ...
        <div className="mt-8 rounded-2xl border bg-white/70 p-5 shadow-sm backdrop-blur">
          <p className="text-sm font-medium opacity-70">Vista previa</p>
          <h4 className="mt-2 text-2xl font-semibold">{brand.loading.message}</h4>
