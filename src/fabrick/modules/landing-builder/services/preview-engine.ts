    ...
    (function bootstrapPreview(){
      ...
      function cleanDomProps(props) {
        ...
        Object.keys(props || {}).forEach(function(key){
          if (!blocked.has(key)) next[key] = props[key];
        });
        return next;
      }
      ...
      function makeIcon(name) {
        return function IconComponent(props) {
          ...
          return React.createElement(
            "svg",
            { ...attrs, className: className, width: size, height: size, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: strokeWidth, strokeLinecap: "round", strokeLinejoin: "round", role: "img", "aria-label": name },
      ...
      const motion = new Proxy({}, {
        get: function(_target, tag) {
          return makePrimitive(String(tag), "");
        }
      });
      ...
      function Link(props) {
        const clean = cleanDomProps(props || {});
        const href = clean.href || "#";
        const children = clean.children;
        delete clean.children;
        return React.createElement("a", { ...clean, href: href }, children);
      }
      ...
      function createGlobe(canvas, options) {
        ...
        function draw(t) {
          ...
          for (let i = -3; i <= 3; i++) {
            ctx.beginPath();
            ctx.ellipse(x, y, r * Math.cos(i * .18), r * .22, t / 900 + i, 0, Math.PI * 2);
            ctx.stroke();
          }
          if (options && typeof options.onRender === "function") options.onRender({ phi: t / 900, theta: .3, width: w, height: h });
          requestAnimationFrame(draw);
        }
        requestAnimationFrame(draw);
        return { destroy: function(){ destroyed = true; }, toggle: function(){}, resize: function(){} };
      }

      const shimGlobals = {
        cn: cn,
        motion: motion,
        framerMotion: { motion: motion, AnimatePresence: AnimatePresence, useReducedMotion: useReducedMotion },
      ...
      try {
        const ReactHooks = { useState: React.useState, useEffect: React.useEffect, useMemo: React.useMemo, useRef: React.useRef, useCallback: React.useCallback, Fragment: React.Fragment };
        Object.assign(window, ReactHooks, shimGlobals);
        ...
        const transformed = Babel.transform(userCode, {
          presets: [["typescript", { isTSX: true, allExtensions: true }], ["env", { modules: false }], "react"],
