...
type TestResult = {
  ...
  preference?: { id?: string; init_point?: string; sandbox_init_point?: string } | null;
};

const STORAGE_KEY = "omnifix_mp_test_credentials_local_v1";
...
export default function MercadoPagoTestClient() {
  ...
  async function runTest(createPreference = false) {
    ...
    if (rememberLocal) {
      try {
        localStorage.setItem(STORAGE_KEY, JSON.stringify({ publicKey, accessToken }));
      } catch {}
    }
    try {
  ...
  return <main className="min-h-screen bg-[#050816] px-4 py-6 text-white md:px-6">
    <section className="mx-auto max-w-7xl space-y-5">
      <header className="overflow-hidden rounded-[2.5rem] border border-cyan-300/20 bg-[radial-gradient(circle_at_80%_0%,rgba(0,245,255,.24),transparent_34rem),linear-gradient(135deg,rgba(255,255,255,.08),rgba(255,255,255,.025))] p-6 shadow-2xl shadow-black/40 md:p-8">
        <Link href="/admin/pagos/mercadopago" className="inline-flex rounded-full border border-white/10 bg-white/10 px-3 py-2 text-xs font-black text-white">← Volver a pagos</Link>
        <p className="mt-8 inline-flex items-center gap-2 rounded-full border border-cyan-300/30 bg-cyan-400/10 px-3 py-1 text-[10px] font-black uppercase tracking-[.28em] text-cyan-100"><FlaskConical className="size-3.5" /> Sandbox Mercado Pago</p>
        <h1 className="mt-5 text-4xl font-black tracking-[-.06em] md:text-6xl">Probar credenciales TEST.</h1>
        <p className="mt-4 max-w-3xl text-sm leading-7 text-zinc-300">Pantalla aislada para validar llaves de prueba. No guarda en base de datos, no reemplaza variables de Vercel y bloquea credenciales productivas.</p>
      </header>

      <section className="grid gap-5 lg:grid-cols-[1fr_420px]">
        <article className="rounded-[2rem] border border-white/10 bg-white/[0.045] p-5">
          <div className="flex items-center gap-3">
            <div className="grid size-12 place-items-center rounded-2xl bg-cyan-300 text-black"><CreditCard className="size-6" /></div>
            <div>
          ...
          <div className="mt-6 grid gap-4">
            <label className="block rounded-2xl border border-white/10 bg-black/35 p-4">
              <span className="text-[10px] font-black uppercase tracking-[.22em] text-cyan-200">Public Key TEST</span>
            ...
            <label className="block rounded-2xl border border-white/10 bg-black/35 p-4">
              <span className="text-[10px] font-black uppercase tracking-[.22em] text-cyan-200">Access Token TEST</span>
              <div className="mt-2 flex items-center gap-2">
                ...
                <button type="button" onClick={() => setShowToken((value) => !value)} className="grid size-10 place-items-center rounded-xl border border-white/10 bg-white/10 text-zinc-300">{showToken ? <EyeOff className="size-4" /> : <Eye className="size-4" />}</button>
              </div>
            </label>
          ...
          {!valid && <div className="mt-5 rounded-2xl border border-amber-300/20 bg-amber-400/10 p-4 text-sm leading-6 text-amber-50"><AlertTriangle className="mr-2 inline size-4" /> Pega un Access Token que comience con TEST-. Esta pantalla bloqueará cualquier token productivo APP_USR-.</div>}

          <div className="mt-6 flex flex-wrap gap-3">
            <button onClick={() => void runTest(false)} disabled={!valid || loading} className="inline-flex min-h-12 items-center gap-2 rounded-2xl bg-cyan-300 px-5 text-sm font-black text-black transition hover:bg-cyan-200 disabled:cursor-not-allowed disabled:opacity-45">{loading ? <Loader2 className="size-4 animate-spin" /> : <ShieldCheck className="size-4" />} Probar conexión</button>
            <button onClick={() => void runTest(true)} disabled={!valid || loading} className="inline-flex min-h-12 items-center gap-2 rounded-2xl border border-white/10 bg-white/10 px-5 text-sm font-black text-white transition hover:bg-white/15 disabled:cursor-not-allowed disabled:opacity-45"><Wallet className="size-4" /> Crear preferencia demo</button>
        ...
        <aside className="space-y-4">
          <article className="rounded-[2rem] border border-white/10 bg-white/[0.045] p-5">
            ...
            {!result && <p className="mt-3 text-sm leading-6 text-zinc-400">Ejecuta una prueba para ver cuenta TEST, latencia y preferencia demo.</p>}
            {result && <div className="mt-4 space-y-3">
              <div className={`rounded-2xl border p-4 ${result.ok ? "border-emerald-300/25 bg-emerald-400/10 text-emerald-50" : "border-red-300/25 bg-red-500/10 text-red-50"}`}>
                {result.ok ? <CheckCircle2 className="mr-2 inline size-4" /> : <AlertTriangle className="mr-2 inline size-4" />}
          ...
          {sandboxUrl && <article className="rounded-[2rem] border border-emerald-300/20 bg-emerald-400/10 p-5">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <a href={sandboxUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-2xl bg-emerald-300 px-4 py-3 text-sm font-black text-black">Abrir checkout TEST <ArrowRight className="size-4" /></a>
              <button onClick={() => void copy(sandboxUrl)} className="inline-flex items-center gap-2 rounded-2xl border border-white/10 bg-white/10 px-4 py-3 text-sm font-black text-white"><Copy className="size-4" /> Copiar link</button>
            </div>
          </article>}
          ...
          <article className="rounded-[2rem] border border-amber-300/20 bg-amber-400/10 p-5 text-sm leading-6 text-amber-50">
            ...
            <p className="mt-2">Esta herramienta no modifica el gateway productivo. Para producción sigue usando las variables oficiales de Vercel.</p>
          </article>
        </aside>
      </section>
    </section>
...
function Info({ label, value }: { label: string; value: string }) {
  return <div className="rounded-2xl border border-white/10 bg-black/35 p-3"><p className="text-[10px] font-black uppercase tracking-[.2em] text-cyan-200">{label}</p><p className="mt-1 break-words text-sm font-bold text-white">{value}</p></div>;
}
