...
export function SetupWizard({ migrationSql }: SetupWizardProps) {
  ...
  const [insforgeAnonKey, setInsforgeAnonKey] = useState("");
  const [insforgeProjectId, setInsforgeProjectId] = useState("");
  ...
  const manualCredentialsComplete =
    provider === "supabase"
      ...
      : Boolean(insforgeBaseUrl && insforgeAnonKey && insforgeProjectId);
          ...
          <CardContent className="space-y-4">
            ...
            {!providerEnvDetected && provider === "insforge" && (
              <>
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="setup-insforge-anon">Anon key</Label>
                  <Input
                    id="setup-insforge-anon"
                    type="password"
                    value={insforgeAnonKey}
                    onChange={(e) => setInsforgeAnonKey(e.target.value)}
                  />
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor="setup-insforge-project">Project ID</Label>
                  <Input
                    id="setup-insforge-project"
                    value={insforgeProjectId}
                    onChange={(e) => {
                      setInsforgeProjectId(e.target.value);
                      setConnectionOk(false);
                    }}
                  />
                </div>
          ...
          <CardContent className="space-y-4">
            <div className="text-sm">
              ...
              <ol className="mt-2 list-decimal space-y-1 pl-4 text-muted-foreground">
                ...
                <li>
                  Abre el <strong>SQL Editor</strong> de tu proyecto en Supabase y pegalo. Si usas InsForge, este paso
                  se marca como verificado automaticamente hasta que exista una migracion especifica del proveedor.
                </li>
